...
public class PFLocalisationNode extends AbstractNodeMain {
    ...
    private Publisher<PoseStamped> estimatedpose;
    ...
    public void onStart(final ConnectedNode connectedNode) {
        ...
        estimatedpose = node.newPublisher("estimatedpose", PoseStamped._TYPE);
        ...
        laser.addMessageListener(new MessageListener<LaserScan>() {
            ...
            public void onNewMessage(LaserScan message) {
                ...
                if (mapReceived && initialPoseReceived && sufficientMovementDetected(pf.getPose())) {
                    ...
                    PoseStamped estimate = estimatedpose.newMessage();
                    estimate.getPose().setPosition(pf.getPose().getPose().getPose().getPosition());
                    estimate.getPose().setOrientation(pf.getPose().getPose().getPose().getOrientation());
                    estimate.getHeader().setFrameId("/map");
                    estimatedpose.publish(estimate);
        ...
        initialpose.addMessageListener(new MessageListener<PoseWithCovarianceStamped>() {
            ...
            public void onNewMessage(PoseWithCovarianceStamped message) {
                ...
                last_published_pose = node.getTopicMessageFactory().newFromType(PoseWithCovarianceStamped._TYPE); // Create empty 'last pose'
                particlecloud.publish(pf.getParticleCloud()); // Display particle cloud
            }
        });
