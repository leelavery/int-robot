...
public abstract class AbstractLocaliser extends PFLocalisationNode {
    ...
    protected static double ODOM_ROTATION_NOISE; // Odometry model rotation noise
    protected static double ODOM_TRANSLATION_NOISE; // Odometry model x axis (forward) noise
    protected static double ODOM_DRIFT_NOISE; // Odometry model y axis (side-to-side) noise
    ...
    public AbstractLocaliser() {    
        ...
        estimatedpose.getPose().getPose().getPosition().setY(INIT_Y);
        estimatedpose.getPose().getPose().getPosition().setZ(INIT_Z);
     ...
     */
    public abstract Pose estimatePose(PoseArray particlecloud);
    ...
    public PoseWithCovarianceStamped updatePoseStamped(PoseArray particlecloud) {
        ...
        p.getPose().setPose(estimatePose(particlecloud)); // Get Pose from the implementing class
    ...
    public void updateOdom(TransformStamped odomTf) {
        ...
        for (Pose p : particlecloud.getPoses()) {
            ...
            // Rotate particle according to odometry rotation, plus some noise
            p.setOrientation(rotateQuaternion(p.getOrientation(), dif_heading + rnd * dif_heading * ODOM_ROTATION_NOISE));
            
            // Get particle's new orientation
            double theta = getHeading(p.getOrientation());
            
            // Find translation in the direction of the particle's orientation
            double travel_x = distance_travelled * Math.cos(theta);
            double travel_y = distance_travelled * Math.sin(theta);
            p.getPosition().setX(p.getPosition().getX() + travel_x + (rnd * travel_x * ODOM_TRANSLATION_NOISE));
            p.getPosition().setY(p.getPosition().getY() + travel_y + (rnd * travel_y * ODOM_DRIFT_NOISE));
        }
    }
